...
function ShowRecommendedProduct({category,heading}) {
  ...
  return (
    <div className='container px-2 my-2 mx-auto'>
        <h2 className="text-2xl font-semibold py-3">{heading}</h2>
        {
          loading ? (
            ...
            <div className="grid grid-cols-[repeat(auto-fit,minmax(300px,320px))] justify-between gap-6 md:gap-6 overflow-scroll overflow-y-hidden scrollbar-hide scroll-smooth transition-all px-2">
              {
                product.map((ele,index)=>(
                  <Link to={"/product/"+ele?._id} className="w-full min-w-[280px] md:min-w-[320px] max-w-[280px] md:max-w-[320px] bg-white shadow-lg rounded-lg mb-4 md:mb-6" key={index+1} onClick={()=>scrollTop()}>
...
export default ShowRecommendedProduct