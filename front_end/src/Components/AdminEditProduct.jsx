...
function AdminEditProduct({onClose,Data,fetchAllProduct}) {
    ...
    // upload product on data base
     const handleEditProduct=async(e)=>{
      e.preventDefault()
      try {
        const respose= await axios.put(`${import.meta.env.VITE_BASE_URL}/api/v1/products/updateProduct/${Data._id}`,productData,{withCredentials:true})
        ...
        fetchAllProduct()
         ...
         toast.error("Product Edition Faild!")
      }
     }
     ...
     <div className={"fixed w-full top-0 left-0 right-0 bottom-0 flex justify-center items-center bg-slate-200 bg-opacity-80 z-50"}>
       <div className={"bg-white p-2 rounded w-full max-w-xl h-full max-h-[65%] overflow-hidden "}>
         <div className='flex justify-between items-center'>
            <CiEdit className='text-2xl text-gray-700 mx-2'/>
         ...
         <form className='grid p-4 gap-1 overflow-y-scroll h-full pb-8'>
           ...
           <button className='px-2 py-1 bg-gray-700 text-white mt-2 rounded-lg hover:bg-gray-600' onClick={handleEditProduct}>Update Product</button>
           <Toaster/>
         </form>
